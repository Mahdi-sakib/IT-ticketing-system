// Lazily populates the local database with realistic demo data the first
// time the app runs (see the useEffect in App.jsx). Everything here is
// fictional and safe to reset at any time from Admin > System Settings.
import {
  Users, Assets, ConfigurationItems, ChangeRequests, Problems, MajorIncidents,
  KnowledgeBaseArticles, AutomationRules, SlaPolicies, Tickets, TicketComments,
  TicketHistory, CsatResponses,
} from "@/api/entities";
import { nextTicketId, slaDueDate, SLA_POLICY } from "@/lib/ticketConstants";
...
export function seedDatabase() {
  ...
  // ---- CMDB ------------------------------------------------------------
  const ciApp = ConfigurationItems.create({
  ...
  kb.forEach((a, i) => KnowledgeBaseArticles.create({ ...a, status: "published", author_name: i % 2 ? agent2.full_name : agent1.full_name, view_count: 12 + i * 7, helpful_count: 3 + i }));

  // ---- Automation Rules ------------------------------------------------
  AutomationRules.create({ name: "Escalate Cybersecurity Tickets", description: "Automatically mark cybersecurity tickets as critical and route to Security.", trigger_type: "ticket_created", conditions: "category=Cybersecurity", actions: "priority=critical; assign_team=Security Team", priority_order: 1, enabled: true, last_run: null, run_count: 0 });
  AutomationRules.create({ name: "Route VPN Issues to Network Support", description: "Route VPN category tickets straight to the Network Support team.", trigger_type: "ticket_created", conditions: "category=VPN", actions: "assign_team=Network Support", priority_order: 2, enabled: true, last_run: null, run_count: 0 });
  AutomationRules.create({ name: "Flag Server Outages as Critical", description: "Server-related tickets are automatically treated as critical priority.", trigger_type: "ticket_created", conditions: "category=Server", actions: "priority=critical", priority_order: 3, enabled: true, last_run: null, run_count: 0 });

  // ---- SLA Policy reference rows -----------------------------------------
  Object.entries(SLA_POLICY).forEach(([priority, policy]) =>
    SlaPolicies.create({ priority, response_minutes: policy.response, resolution_minutes: policy.resolution })
  );
  ...
  specs.forEach((s, i) => {
    ...
    const ticket = Tickets.create(fields);

    TicketHistory.create({ ticket_id: ticket.id, action: "Ticket created", actor_name: s.requester.full_name, created_date: createdAt });
    if (s.agent) {
      TicketHistory.create({ ticket_id: ticket.id, action: `Assigned to ${s.agent.full_name}`, actor_name: "System", created_date: createdAt });
    }
    if (s.resolve) {
      TicketHistory.create({ ticket_id: ticket.id, action: "Ticket resolved", actor_name: s.agent?.full_name || "System", created_date: fields.resolved_at });
    }

    if (i < 4) {
      TicketComments.create({ ticket_id: ticket.id, author_id: s.requester.id, author_name: s.requester.full_name, author_role: "employee", body: "Any update on this? It's affecting my work.", is_internal: false, created_date: createdAt });
      if (s.agent) {
        TicketComments.create({ ticket_id: ticket.id, author_id: s.agent.id, author_name: s.agent.full_name, author_role: "agent", body: "Thanks for reporting this — we're looking into it now and will update you shortly.", is_internal: false, created_date: new Date(new Date(createdAt).getTime() + 30 * 60000).toISOString() });
        TicketComments.create({ ticket_id: ticket.id, author_id: s.agent.id, author_name: s.agent.full_name, author_role: "agent", body: "Internal note: checked logs, likely a config issue upstream.", is_internal: true, created_date: new Date(new Date(createdAt).getTime() + 45 * 60000).toISOString() });
      }
    }
